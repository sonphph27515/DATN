...
<div class="container mt-3">
    ...
    <div class="row mt-3">
        <div class="col-9">
            <table class="table table-bordered mt-3 text-center">
                ...
                <tbody>
                <c:forEach varStatus="index" items="${clPage.content}" var="th">
                    <tr>
                        <td>${index.index + clPage.number * clPage.size + 1}</td>
                        <td>${th.ten}</td>
                        <td>
        ...
        <div class="col-3 mt-4">
            <form:form id="edit-form" modelAttribute="cl" method="post" action="/admin/chat-lieu/store">
    ...
    <div class="mt-3">
        <div class="text-center">
            <c:if test="${clPage.totalPages > 1}">
                <ul class="pagination">
                    <li class="page-item <c:if test="${clPage.number == 0}">disabled</c:if>">
                        <a class="page-link" href="?page=1">First</a>
                    </li>
                    <c:forEach var="i" begin="1" end="${clPage.totalPages}">
                        <li class="page-item <c:if test="${clPage.number + 1 == i}">active</c:if>">
                            <a class="page-link" href="?page=${i}">${i}</a>
                        </li>
                    </c:forEach>
                    <li class="page-item <c:if test="${clPage.number == clPage.totalPages - 1}">disabled</c:if>">
                        <a class="page-link" href="?page=${clPage.totalPages}">Last</a>
                    </li>
                </ul>
            </c:if>
        </div>
...
<script>
    $(".update-button").click(function () {
        ...
        $.ajax({
            url: "/admin/chat-lieu/get/" + id,
            type: "GET",
            success: function (data) {
                $("#ten").val(data.ten);
                $("#trang_thai").val(data.trang_thai);

                $("#edit-form").attr("action", "/admin/chat-lieu/update/" + id);
            },
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        updateButtons.forEach(function (button) {
            button.addEventListener("click", function () {
                ...
                modalTitle.textContent = "Cập nhật Chất liệu";
        ...
        clickClose.addEventListener("click", function () {
            ...
            modalTitle.textContent = "Thêm Chất liệu";

            $("#edit-form").trigger("reset");
            $("#edit-form").attr("action", "/admin/chat-lieu/store");
